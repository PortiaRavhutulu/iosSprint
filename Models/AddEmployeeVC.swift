...
class AddEmployeeVC: UIViewController {
    ...
    @IBAction func btnAddEmployee(_ sender: UIButton) {
        ...
        if (firstName.text?.isEmpty != false || ((lastName.text?.isEmpty) != false) || ((cellNumber.text?.isEmpty) != false) || ((email.text?.isEmpty) != false) || ((role.text?.isEmpty) != false) || ((salary.text?.isEmpty) != false)){
            createEmptyFieldsErrorAlert()
        }
        else if (cellNumber.text?.count ?? 0 < 10 || cellNumber.text?.count ?? 0 > 10){
            createInvalidCellErrorAlert()
        }
        else if (email.text?.contains("@") == false){
            createInvalidEmailErrorAlert()
        }
        else {
            postRequest()
            createSuccessAlert()
    ...
    func createInvalidCellErrorAlert() {
        let alert = UIAlertController(title: "Error", message: "Please enter a valid cell number", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: {_ in print("OK tap")}))
        
        present(alert, animated: true, completion: nil)
    }
    
    func createInvalidEmailErrorAlert() {
    ...
    func createEmptyFieldsErrorAlert() {
